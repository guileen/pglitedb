...
func TestValueEncoding(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil", nil, types.ColumnTypeString},
		{"string", "hello world", types.ColumnTypeString},
		{"empty string", "", types.ColumnTypeString},
		{"int", int64(42), types.ColumnTypeNumber},
		{"negative int", int64(-42), types.ColumnTypeNumber},
		{"zero", int64(0), types.ColumnTypeNumber},
		{"float", float64(3.14), types.ColumnTypeNumber},
		{"bool true", true, types.ColumnTypeBoolean},
		{"bool false", false, types.ColumnTypeBoolean},
		{"timestamp", time.Now(), types.ColumnTypeTimestamp},
		{"bytes", []byte{1, 2, 3, 4}, types.ColumnTypeBinary},
	}
...
func TestStringValueOrdering(t *testing.T) {
	...
	for i, v := range values {
		encoded, err := codec.EncodeValue(v, types.ColumnTypeString)
		if err != nil {
			t.Fatalf("encode failed: %v", err)
		}
...
func TestNumberValueOrdering(t *testing.T) {
	...
	for i, v := range intValues {
		encoded, err := codec.EncodeValue(v, types.ColumnTypeNumber)
		if err != nil {
			t.Fatalf("encode failed: %v", err)
		}
...
func TestCompositeKeyEncoding(t *testing.T) {
	...
	types := []types.ColumnType{
		...
		types.ColumnTypeNumber,
		types.ColumnTypeBoolean,
	}
...
func BenchmarkEncodeValue(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		codec.EncodeValue(value, types.ColumnTypeString)
	}
}

...
func TestQueryExecutor_Select(t *testing.T) {
	...
	row := result.Rows[0]
	if row.Data["name"].Data != "Alice" {
		t.Errorf("expected name=Alice, got %v", row.Data["name"].Data)
	}

	if row.Data["email"].Data != "alice@example.com" {
		t.Errorf("expected email=alice@example.com, got %v", row.Data["email"].Data)
	}
}
...
func TestQueryExecutor_OrderBy(t *testing.T) {
	...
	for i, row := range result.Rows {
		name, _ := row.Data["name"].Data.(string)

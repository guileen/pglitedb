...
func TestSnapshotTransaction_UpdateRows(t *testing.T) {
	...
	for i := int64(1); i <= 3; i++ {
		row := &types.Record{
			Data: map[string]*types.Value{
				...
				"age":  {Data: 20 + int(i), Type: types.ColumnTypeNumber},
			},
			CreatedAt: time.Now(),
			UpdatedAt: time.Now(),
		}
		_, err := engine.InsertRow(ctx, 1, 1, row, schemaDef)
		require.NoError(t, err)
	}
	...
	record, err := engine.GetRow(ctx, 1, 1, 1, schemaDef)
	require.NoError(t, err)
	assert.Equal(t, 30, record.Data["age"].Data)

	// Verify other rows are unchanged
	record, err = engine.GetRow(ctx, 1, 1, 2, schemaDef)
	require.NoError(t, err)
	assert.Equal(t, 22, record.Data["age"].Data)
}
...
func TestSnapshotTransaction_DeleteRows(t *testing.T) {
	...
	_, err = engine.GetRow(ctx, 1, 1, 1, schemaDef)
	assert.Error(t, err)
	assert.Equal(t, types.ErrRecordNotFound, err)

	// Verify other rows still exist
	_, err = engine.GetRow(ctx, 1, 1, 2, schemaDef)
	require.NoError(t, err)
}
...
func (m *tableManager) findIndexForColumn(schema *table.TableDefinition, columnName string) int64 {
	for i, index := range schema.Indexes {
		// Check if this is a single-column index matching our column
		if len(index.Columns) == 1 && index.Columns[0] == columnName {
			return int64(i + 1) // Index IDs are 1-based
		}
	}
	return 0 // No matching index found
}
